...
const ProjectCreate = () => {
  ...
  const validateProject = () => {
    ...
    if (Object.keys(project.countryLanguages).length === 0) {
  ...
  const handleTemplateSelect = (templateQuestion) => {
    const newQuestion = {
      ...templateQuestion,
      questionId: `S${project.questions.length + 1}`
    };
  ...
  const handleSubmit = async () => {
    if (!validateProject()) {
      alert('Please fix the validation errors before creating the project');
      return;
    }

    setLoading(true);
    try {
      await axios.post(`${config.API_BASE_URL}/projects`, project, {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <TemplateSidebar 
        ...
        projectLanguages={Object.values(project.countryLanguages).flat()}
      />
        ...
        <div className="fade-in">
          <div style={{ marginBottom: '24px' }}>
            ...
            <p style={{ color: '#9ca3af', marginTop: '8px', fontSize: '14px' }}>
              Build your survey with advanced question logic and multi-language support
            </p>
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., SURVEY_2024_Q1"
                  style={{
                    width: '100%',
                    padding: '12px',
                    background: 'rgba(0, 0, 0, 0.3)',
                    border: `1px solid ${errors.projectCode ? '#ef4444' : '#533483'}`,
                    borderRadius: '6px',
                    color: 'white',
                    fontSize: '14px'
                  }}
                />
                {errors.projectCode && <div style={{ color: '#ef4444', fontSize: '12px', marginTop: '4px' }}>{errors.projectCode}</div>}
              </div>
...
onClick={() => setShowQuestionBuilder(true)}
            ...
            {errors.questions && <div style={{ color: '#ef4444', fontSize: '12px', marginBottom: '16px' }}>{errors.questions}</div>}
...
onClick={() => setShowQuestionBuilder(true)}
          ...
          {showQuestionBuilder && (
            <QuestionBuilder
              ...
              questionIndex={editingQuestion !== null ? editingQuestion : project.questions.length}
            />
          )}
          ...
          <div style={{ textAlign: 'center', marginTop: '40px' }}>
            <button 
              ...
              style={{
                padding: '16px 32px',
                background: loading ? '#374151' : 'linear-gradient(135deg, #533483 0%, #0f3460 100%)',
                color: 'white',
                border: 'none',
                borderRadius: '8px',
